...
import { listarRrhhMetrics, guardarRrhhMetric, eliminarRrhhMetric } from '../lib/rrhhMetricsApi';
...
// La meta es fija (la misma que ya tenía definida el KPI), no un dato que
...
const TIPOS = {
  tiempo_respuesta: {
    label: 'Tiempo de Respuesta ante Solicitud de Personal',
    campos: [
      { id: 'ingresadas', label: 'Cant. de Personas ingresadas ante la solicitud ≤ 60 días', type: 'number' },
      { id: 'solicitadas', label: 'Cantidad de Personas solicitadas', type: 'number' },
    ],
    columnas: (d) => {
      const ind = pct(d.ingresadas, d.solicitadas);
      return [fmtNum(d.ingresadas), fmtNum(d.solicitadas), cumpleBadge(fmtPct(ind), ind === null ? null : ind >= 95), 'Meta ≥ 95%'];
    },
    headers: ['Ingresadas ≤60 días', 'Solicitadas', 'Indicador', 'Meta'],
  },
  observaciones: {
    label: 'Observaciones',
    campos: [
      { id: 'observaciones', label: 'Observación', type: 'textarea' },
    ],
...
const RrhhMetricsForms = () => {
  const [tipoActivo, setTipoActivo] = useState('tiempo_respuesta');
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [form, setForm] = useState(emptyForm('tiempo_respuesta'));
  const [saveMsg, setSaveMsg] = useState('');

  const cargarFilas = (tipoKey) => {
    setLoading(true);
    listarRrhhMetrics(tipoKey)
      .then(setRows)
  ...
  const handleSubmit = (e) => {
    ...
    guardarRrhhMetric(tipoActivo, editingId, fecha, data)
      .then(() => {
  ...
  return (
    <div className="module-container">
      <div className="module-header animate-fade-in">
        <div>
          <h2>Carga de Métricas - Gerencia de Recursos Humanos</h2>
...
export default RrhhMetricsForms;
