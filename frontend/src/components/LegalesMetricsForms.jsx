...
import { listarLegalesMetrics, guardarLegalesMetric, eliminarLegalesMetric } from '../lib/legalesMetricsApi';
...
const TIPOS = {
  respuestas_gc: {
    label: 'Respuestas a GC',
    campos: [
      { id: 'resp_comunicadas', label: 'Cant. respuestas comunicadas dentro del plazo (7 días)', type: 'number' },
      { id: 'resp_solicitadas', label: 'Cantidad total de respuestas solicitadas', type: 'number' },
    ],
    columnas: (d) => {
      const ind = pct(d.resp_comunicadas, d.resp_solicitadas);
      return [fmtNum(d.resp_comunicadas), fmtNum(d.resp_solicitadas), cumpleBadge(fmtPct(ind), ind === null ? null : ind >= 95), 'Meta ≥ 95%'];
    },
    headers: ['Comunicadas', 'Solicitadas', 'Indicador', 'Meta'],
  },
  matriz_legal: {
    label: 'Cumplimiento Matriz Legal',
    campos: [
      { id: 'cumplimiento_matriz', label: 'Cumplimiento confección Matriz Legal (0 o 1)', type: 'number' },
    ],
    columnas: (d) => {
      const v = d.cumplimiento_matriz !== null && d.cumplimiento_matriz !== undefined && d.cumplimiento_matriz !== '' ? Number(d.cumplimiento_matriz) : null;
      return [cumpleBadge(v === null ? '-' : fmtNum(v), v === null ? null : v === 1), 'Meta = 1'];
    },
    headers: ['Cumplimiento', 'Meta'],
  },
  observaciones: {
    label: 'Observaciones',
    campos: [
      { id: 'observaciones', label: 'Observaciones / Comentarios Adicionales', type: 'textarea' },
    ],
    columnas: (d) => [d.observaciones || '-'],
    headers: ['Observación'],
  },
};
...
const LegalesMetricsForms = () => {
  const [tipoActivo, setTipoActivo] = useState('respuestas_gc');
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [form, setForm] = useState(emptyForm('respuestas_gc'));
  const [saveMsg, setSaveMsg] = useState('');

  const cargarFilas = (tipoKey) => {
    setLoading(true);
    listarLegalesMetrics(tipoKey)
      .then(setRows)
  ...
  const handleDelete = (row) => {
    ...
    eliminarLegalesMetric(row.id).then(() => cargarFilas(tipoActivo)).catch(err => alert('Error al eliminar: ' + err.message));
  };
  ...
  const handleSubmit = (e) => {
    ...
    guardarLegalesMetric(tipoActivo, editingId, fecha, data)
      .then(() => {
  ...
  return (
    <div className="module-container">
      <div className="module-header animate-fade-in">
        <div>
          <h2>Carga de Métricas - Gerencia de Asuntos Legales</h2>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {Object.entries(TIPOS).map(([key, t]) => (
          <button key={key} type="button" className={tipoActivo === key ? 'btn btn-primary' : 'btn btn-secondary'} onClick={() => setTipoActivo(key)} style={{ fontSize: '13px' }}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
            ...
            {config.campos.map(campo => (
              <div key={campo.id} className="form-group" style={{ marginBottom: 0, minWidth: '220px', flex: campo.type === 'textarea' ? '1 1 100%' : '1 1 220px' }}>
                <label className="form-label">{campo.label}</label>
                {campo.type === 'textarea' ? (
                  <textarea className="form-control" rows="2" placeholder="Valor..." value={form[campo.id] !== undefined ? form[campo.id] : ''} onChange={e => handleChange(campo.id, e.target.value)} />
                ) : (
        ...
        {loading ? (
          ...
          <table className="data-table" style={{ minWidth: '600px' }}>
            ...
            <tbody>
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={config.headers.length + 2} style={{ textAlign: 'center', padding: '32px' }}>No hay cargas todavía para {config.label}.</td>
                </tr>
              )}
            </tbody>
          </table>
...
export default LegalesMetricsForms;
